...
class PayrollManagers extends Page implements HasTable
{
    use InteractsWithTable;
    protected static ?string $navigationIcon = 'heroicon-o-document-text';
    protected static string $view = 'filament.client.pages.settings.admins';
    protected static bool $shouldRegisterNavigation = false;
    protected static ?string $title = 'Payroll Managers';
    protected static ?string $slug = 'settings/payroll-managers';
            ...
            Actions\CreateAction::make()
                ...
                ->action(function (array $data) {
                    ...
                    foreach ($emails as $email) {
                        ...
                        $token = Str::random(60);
                        $role = Filament::getTenant()->roles()->where('name', 'Payroll Manager')->first();
        ...
        return $table
            ->query(
                Filament::getTenant()
                    ->users()
                    ->getQuery()
                    ->whereHas('roles', fn($q) => $q->where('name', 'Payroll Manager'))
            )
            ->columns([
